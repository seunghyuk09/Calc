...
npm run test:e2e         # 실제 Chromium E2E 테스트 (131종, http 모드)
npm run test:standalone  # 단일 파일을 file:// 로 열어 같은 131종 실행
npm test                 # 전부
```
...
**좌우로 쓸면 탭이 넘어갑니다.** 다음 경우에는 스와이프로 보지 않습니다.

- 세로 이동이 가로의 2/3 를 넘을 때 (스크롤로 봅니다)
- 입력창·낙서판·가로 스크롤 영역에서 시작한 제스처
- 드래그로 글자를 고르는 중일 때
- 양 끝 탭에서 바깥으로 쓸 때 (순환하지 않습니다)

스와이프로 끝난 제스처의 클릭은 삼킵니다. 없으면 계산기 키를 누른 채 쓸었을 때
숫자가 입력되면서 탭까지 바뀝니다.
