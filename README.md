...
npm run test:unit        # 단위 테스트 44종 (계산 엔진 9 + 기간 13 + 다국어 15 + 오늘 7)
npm run test:assets      # 정적 파일 참조 무결성 + iOS 아이콘 검사
npm run test:e2e         # 실제 Chromium E2E 테스트 (145종, http 모드)
npm run test:standalone  # 단일 파일을 file:// 로 열어 같은 145종 실행
npm test                 # 전부
```
...
| 기능 | 상태 | 비고 |
|---|---|---|
| **오늘 (플래너 첫 페이지)** | ✅ 완전 동작 | 앱을 열면 첫 화면. 미완료 할 일 자동 순환 + 현재 날씨 요약, 눌러서 상세로 이동 |
| 계산기 | ✅ 완전 동작 | `eval()` 미사용. Shunting-yard 파서 직접 구현, 단위 테스트 9종. 폰은 자체 자판만, PC는 키보드 전체 입력. 기록에 메모 |
...
앱을 열면 **오늘** 탭이 먼저 뜹니다. (마지막에 보던 탭이 있으면 그쪽으로 엽니다)

| 카드 | 내용 | 누르면 |
|---|---|---|
| 현재 날씨 | WMO 아이콘 + 기온 + 최저/최고 | 날씨 탭의 주간 예보로 이동 |
| 오늘 할 일 | 미완료 항목이 3.5초마다 한 줄씩 순환 | 계획표에서 그 항목을 찾아 강조 |

'오늘 할 일'에 모이는 것은 **지금 기간에 해당하는 미완료 항목**입니다.
일간은 오늘, 주간은 이번 주, 월간은 이번 달, 연간은 올해 것까지 포함하고 짧은 주기부터 보여줍니다.
(주간 계획만 쓰는 사람에게 빈 화면을 보여주지 않기 위해서입니다. 왼쪽 색 점이 단위를 나타냅니다)

**좌우로 쓸면 화면이 손가락을 따라 밀리며 넘어갑니다.**

탭만 바뀌는 게 아니라 페이지가 실제로 이동합니다. 제스처를 직접 계산하지 않고
**브라우저의 네이티브 가로 스크롤 스냅**에 맡깁니다. 직접 계산하면 손을 떼야 넘어가고
관성도 없어서 종이 넘기는 느낌이 나지 않습니다.

```
.app         height: 100dvh, overflow: hidden   (헤더 고정)
 └ .app-main display: flex, overflow-x: auto, scroll-snap-type: x mandatory
    └ .panel flex: 0 0 100%, overflow-x: hidden, overflow-y: auto
```

패널 10장이 가로로 나란히 놓이고, 각 패널이 자기 세로 스크롤을 갖습니다.
탭 버튼을 누르면 해당 위치로 스크롤하고, 손으로 쓸면 스크롤이 멈춘 뒤 탭 상태가 따라옵니다.

#### 이 구조에서 반드시 밟는 함정 3개

| 증상 | 원인 | 대응 |
|---|---|---|
| 스와이프가 아예 안 먹음 | `.panel` 에 `overflow-y: auto` 만 주면 `overflow-x` 가 `visible → auto` 로 계산돼 패널이 가로 스와이프를 삼킵니다 | `overflow-x: hidden` 을 명시 |
| **첫 장에서 오른쪽으로 쓸면 앱을 벗어남** | 브라우저가 가로 페이저 끝의 오버스크롤을 '뒤로가기'로 받아들입니다. 히스토리가 없으면 `about:blank` 로 나갑니다. **`overscroll-behavior` 로는 막히지 않습니다** | 양 끝에서 바깥으로 향하는 `touchmove` 만 `preventDefault` (`passive: false` 필수) |
| 낙서 중에 페이지가 넘어감 | — | `#draw-canvas { touch-action: none }` 이 이미 막고 있습니다 |

#### 자동 순환과 접근성

자동으로 움직이는 화면은 읽으려는 순간 지나가버립니다. 그래서:

- 손을 올리거나 포커스가 들어오면 멈춥니다
- **일시정지 버튼**을 따로 둡니다 (WCAG 2.2.2 Pause, Stop, Hide)
- OS 의 **'동작 줄이기'** 설정이 켜져 있으면 자동 순환을 아예 하지 않습니다
- 다른 탭으로 가거나 앱이 백그라운드로 가면 타이머를 끕니다
- 멈춘 상태에서도 손으로 굴려 전부 볼 수 있습니다 (transform 이 아니라 실제 스크롤을 씁니다)

### 계산기: 폰은 자체 자판, PC는 키보드

**폰에서는 시스템 키보드를 띄우지 않습니다.** 올라오면 계산기 자판을 가리기 때문입니다.
키패드를 누를 때마다 `focus()` 가 걸리므로, 입력칸을 누르지 않아도 키보드가 올라오던 문제였습니다.

`(pointer: coarse)` 인 기기에서만 `inputmode="none"` 을 겁니다.

- 커서는 그대로 보여서 어디에 입력되는지 알 수 있습니다
- **물리 키보드 입력은 막지 않습니다.** 태블릿에 키보드를 붙여 써도 그대로 동작합니다
- 태블릿에 키보드를 붙였다 떼면 `matchMedia` 변화를 받아 다시 맞춥니다

PC(`pointer: fine`)에서는 `inputmode="text"` 이고, 수식창을 클릭하지 않아도
숫자·`( )`·`+ - * /`·`%`·`.` 을 바로 칠 수 있습니다. `Enter`/`=` 계산, `Backspace` 지우기, `Esc` 초기화.

### 계산 기록 메모

기록 한 줄마다 "왜 이 계산을 했는지" 적어 둘 수 있습니다. ✎ 를 누르면 입력칸이 열립니다.

- 값을 수식에 넣는 버튼과 메모 버튼을 **따로** 뒀습니다. 겹치면 메모하려다 값이 들어갑니다
- 메모를 비우고 저장하면 지워집니다
- 메모가 있는 줄은 아이콘이 ✎ → 📝 로 바뀌어 한눈에 구분됩니다
- 최대 120자, 기록 40건까지 보관
...
### 데스크톱 트랙패드 가로 스크롤은 검증하지 못했습니다

스와이프는 **터치 이벤트로 실제 검증**했습니다(E2E 가 CDP 로 손가락 제스처를 보냅니다).
하지만 데스크톱 트랙패드의 두 손가락 가로 스크롤은 확인하지 못했습니다.
헤드리스 Chromium 이 합성 휠 이벤트를 `scroll-snap` 과 함께 처리하지 못해
자동화로 검증할 방법이 없었습니다. (스냅을 끄면 휠이 동작하는 것으로 원인을 특정했습니다)

데스크톱에서는 탭 버튼으로 이동할 수 있으므로 기능이 막히지는 않습니다.

### 한/영 전환의 적용 범위

설정 탭의 언어 전환은 **TO DO 탭과 오늘 탭**에만 적용됩니다.
나머지 화면(계산기·날씨·시계·메모·글귀·음악·AI·설정)은 한국어 고정입니다.

오늘 탭의 날씨 설명(WMO)은 한/영 양쪽이 있지만, **날씨 탭 쪽은 한국어로 둡니다.**
설명만 영어로 바꾸면 '습도·강수·바람' 같은 나머지 라벨과 뒤섞여 더 어색해지기 때문입니다.
날씨 탭 전체를 번역하면 그때 함께 바꾸면 됩니다.
...
public/
  ...
  js/
    ...
    lib/
      ...
      i18n.js             TO DO·오늘 탭의 한/영 문구 사전 (전환 UI 는 설정 탭)
      nav.js              탭 이동 중계 (모듈 간 순환 참조 방지)
      store.js            localStorage 래퍼 (차단 환경 대비 폴백 포함)
      dom.js              DOM 헬퍼
    modules/              기능별 모듈 10개 (today.js 가 '오늘' 탭)
